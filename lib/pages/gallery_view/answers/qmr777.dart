...
class Qmr777App extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('qmr777 Demo')),
      body: GalleryView(
        itemCount: 100,
    ...
    return GestureDetector(
      onScaleUpdate: (details) {
        if ((details.scale < 1 &&
            ...
            (details.scale > 1 &&
                _currentColumnCount <= widget.minColumnCount)) {
          return;
        }
        if (details.scale != _scaleRatio) {
          int cc = _calcColumnCount(_scaleRatio);
          setState(() {
            _scaleRatio = details.scale;
            if (_currentColumnCount != cc) {
              _modifyCount++;
              _interpolation *= cc / _currentColumnCount;
              _tmpColumnCount = _currentColumnCount;
              _currentColumnCount = cc;
            }
          });
        }
      },
      ...
      child: Transform(
        ...
        child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              Widget newV = widget.builder(context, index);
              double begin = prevIndex == index ? 1 : 0;
              return TweenAnimationBuilder<double>(
                  ...
                  builder: (context, value, _) {
                    return Stack(
                      fit: StackFit.expand,
                      children: [
                        Opacity(opacity: value, child: newV),
                        Opacity(opacity: 1 - value, child: old),
                      ],
                    );
                  });
            }),
